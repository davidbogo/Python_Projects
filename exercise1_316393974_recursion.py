...
def translate(rna_seq):
    ...
    for cur_shift in range(0, 3):  # we check all 3 possible frame shifts
        ...
        if idx == -1:  # if we haven't found AUG, we break the loop, because what is next is not relevant
            break
        for codon_start_index in range(idx + 3, len(rna) - 3, 3):  # we check and create a sequence of nucleotides
            ...
            if cur_codon == 'UAG' or cur_codon == 'UAA' or cur_codon == 'UGA':  # end codons
                ...
                if len(rna_longest_seq) > len(remaining_sequence[codon_start_index + 3:]):
                    break
                seq_after_end_codon = remaining_sequence[codon_start_index + 3:]
                recursion_check_for_long_seq_after_end_codon = translate(seq_after_end_codon)
                if len(recursion_check_for_long_seq_after_end_codon) > len(rna_longest_seq):
                    rna_longest_seq = recursion_check_for_long_seq_after_end_codon
                break
